...
2. **你的应用连接** `0700` 目录里的 `0600` 套接字 —— 没有 TCP、没有 root、没有公网入口。`arsd` 读取
   ...
   按 owner 归属：只有拥有它们的调用方能查询、跟随、取消自己的 Run，以及查看自己 Session 的状态与列表。
...
**被重放的历史不是本次 Run 的证据。** agent 在服务 `session/load` 时重放整段对话，每一帧仍会先对照期望的
Session 身份完成校验，随后与当前 Run 分离：重放不贡献任何逐事件执行证据、不计入权限中介账目、不参与工具
调用闭合，也不进入 `final_message` —— 因此在更早 Run 里就已完成的工具调用，绝不会被记到本次 Run 的冻结
授权头上。观测到重放的 Run 只保留恰好一条有界的 `session_replay_summary` 事件，其中只有聚合计数，不含任何
被重放的内容。

**当前 Run 的证据经由一条有界串行账本（bounded serial ledger）准入并落盘。** 事件在被接受的那一刻就拿到
它真正的 `seq` 与最终的 NDJSON 行，因此被计的条数与精确字节数就是最终写出的那些，并且在持久化 append 返回
之前一直计着。条数上限与字节上限各自独立生效 —— 1024 → 8192 条事件，以及 8 → 64 MiB —— 且只有在落盘侧确实
取得持久化进展时才会扩张。账本临时写满时，每个已接受事件只有一个绝对的 FIFO 截止时间；停滞或失败的证据落盘
端不会被更大的缓冲遮掩，而是失败关闭，该 Run 报出一个有界的 `EVIDENCE_PIPELINE` 失败。
