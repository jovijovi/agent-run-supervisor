"""Static, shape- and key-based redaction of free-form text and mappings.

Every rule here is a **shape** rule: a credential-shaped literal, or a mapping
key whose name marks the value as sensitive. None of them is derived from a
per-Run value set, so none of them needs one to exist.

There is deliberately no per-Run exact-literal matcher any more. ARS keeps
environment values out of its *structured* durable material — the sealed launch
snapshot records a name, its source class, its precedence layer, and its
redaction status, and never a value — but it no longer scans free-form Run text
for the complete literals it handed the child. The consequence is stated rather
than hidden: an AGENT that echoes an arbitrary environment value which matches
none of the patterns below may have that value retained in ARS evidence.
"""
