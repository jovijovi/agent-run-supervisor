...
║          ──── read-only agent_list → canonical ids from the snapshot          ║
...
| Loaded registry roster | ARS daemon snapshot | return stable canonical `agent_id` values through the read-only `agent_list` operation | expose entry configuration, infer health/roles/routing, or reload the registry |
