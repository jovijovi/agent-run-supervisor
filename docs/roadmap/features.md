...
| Parser/status/store | 0 | 3 | 1 | Current run surfaces exist; S1c adds prompt-turn/management parsing and session turn artifacts; S1d adds atomic lifecycle state + close/abort evidence; retention/cleanup remain. |
| CLI | 4 | 1 | 1 | validate/replay/dry-run/real exec done; doctor partial; S1c adds `session create|send|status`; S1d adds `session close|abort|list`. |
| Execution modes | 1 | 1 | 0 | One-shot exec done; persistent sessions have S1a contract evidence, S1b store/lock foundation, the S1c create/send/status runtime MVP, and the S1d close/abort/list lifecycle slice; real-acpx smoke, multi-turn resume, crash-recovery, and retention still open. |
